...
void tablahash_insertar(TablaHash tabla, void *dato)
{
  ...
  if (casilla->sig != NULL)
  {
    printf("No hay mas espacio en la tabla\n");
    return;
  }
  ...
  while (k < capacidad && !insertado)
  {
    ...
    unsigned idx = (casilla->idx + k) % capacidad;
...
void imprimir_tabla(TablaHash tabla)
{
  ...
  {
    printf("[ %p ][ %d ][ %p ]\n", &tabla->elems[i], (tabla->elems[i].dato != NULL) ? *(int *)tabla->elems[i].dato : -1, tabla->elems[i].sig);
  }
}
